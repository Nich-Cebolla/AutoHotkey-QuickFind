...
class QuickFind {
    ...
    static Call(Arr, Value, &OutValue?, Condition := '>=', IndexStart := 1, IndexEnd?, ValueCallback?) {
        ...
        if RightV == LeftV {
            ...
            Debug_Lines_2(A_LineNumber, A_ThisFunc, '1 Unique val; Entered 1 Unique val. Val: ' RightV)
            switch Condition {
                case '>':
                    if LeftV <= Value {
                        return
            ...
            Debug_Lines_2(A_LineNumber, A_ThisFunc, '1 Unique val; Value satisfies condition.')
            if Right == Left {
                OutValue := Arr[Right]
                return Right
            }
            ...
            ; `Right`.
            Debug_Lines_2(A_LineNumber, A_ThisFunc, '1 Unique val; Multiple indices with value.')
            i := 0
            while !Arr.Has(++i) {
                continue
            }
            ...
            RightV := GetValue()
            Debug_Lines_2(A_LineNumber, A_ThisFunc, '1 Unique val; LeftV: ' LeftV ' RightV: ' RightV)
            if LeftV == RightV {
        ...
        Debug_Lines_2(A_LineNumber, A_ThisFunc, 'StopBinary: ' StopBinary)
        ; set breakpoint
        if qf_debug.pause
            qf_debug.pause := false
        ...
        loop StopBinary {
            if A_Index == 1 {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Enter loop; Start i: ' i)
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Enter loop; GetValue(): ' (Arr.Has(i) ? GetValue() : ''))
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Enter loop; Right: ' Right '    Left: ' Left)
            } else {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Loop ' A_Index '; Start i: ' i)
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Loop ' A_Index '; GetValue(): ' (Arr.Has(i) ? GetValue() : ''))
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Loop ' A_Index '; Right: ' Right '    Left: ' Left)
            }
            i := Right - Ceil((Right - Left) * 0.5)
            Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Split i: ' i)
            Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; ' (Arr.Has(i) ? 'GetValue() (' GetValue() ') ' (GetValue() > Value ? '>' : GetValue() == Value ? '==' : '<') : '') ' Value (' Value ')')
            while !Arr.Has(i) {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); IndexEnd: ' IndexEnd)
                if i + 1 > IndexEnd {
                    Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); Reversed search direction.')
                    while !Arr.Has(--i) {
                        Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); --i; i ' i)
                        continue
                    }
                    Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); Found i: ' i)
                    Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); GetValue() (' GetValue() ') ' (GetValue() > Value ? '>' : GetValue() == Value ? '==' : '<') ' Value (' Value ')')
                    if Compare_GT() {
                        Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); Entering Sequence_GT.')
                        return Sequence_GT()
                    } else {
                        Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); Entering Sequence_LT.')
                        return Sequence_LT()
                    }
                } else {
                    Debug_Lines_2(A_LineNumber, A_ThisFunc, '!Arr.Has(i==' i '); i++; i ' i)
            ...
            if Compare_EQ() {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Entering HandleEqualValues.')
                return HandleEqualValues()
            }
            if Compare_Loop() {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Compare_Loop() == true. Previous left: ' Left)
                Left := i
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Compare_Loop() == true. New left: ' Left)
            } else {
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Compare_Loop() == false. Previous right: ' Right)
                Right := i
                Debug_Lines_2(A_LineNumber, A_ThisFunc, 'Process loop; Compare_Loop() == false. New right: ' Right)
            }
        }
        ...
        _Sequence_GT_A_1() {
            ...
            ; `Condition`.
            Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Enter')
            loop IndexEnd - i {
                if A_Index == 1 {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Entering loop')
                } else {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Loop ' A_Index)
                }
                if Arr.Has(++i) {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Arr.Has(i)')
                    if Condition() {
                        Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Condition() == true')
                        OutValue := Arr[i]
                        return i
                    }
                }
        ...
        _Sequence_GT_A_2() {
            ...
            loop i - IndexStart {
                ...
                if Arr.Has(--i) {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Arr.Has(i)')
                    if AltCondition() {
                        Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'AltCondition() == true')
                        if EQ && Compare_EQ() {
                            return HandleEqualValues()
                        } else {
        ...
        _HandleEqualValues_EQ() {
            ...
            loop LoopCount {
                if A_Index == 1 {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Enter loop')
                } else {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Loop ' A_Index)
                }
                i += HEV_Direction
                if Arr.Has(i) {
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Arr.Has(i)')
                    if !Compare_EQ() {
                        Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', '!Compare_EQ() - break')
                        break
                    }
                    Previous := i
                    Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Compare_EQ() - continue')
                }
            }
            Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Return Previous: ' Previous)
            OutValue := Arr[Previous]
            return Previous
        }
        ...
        _HandleEqualValues_NEQ() {
            ...
            if Arr.Has(i) {
                Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Return i: ' i)
                OutValue := Arr[i]
                return i
            }
            Debug_Lines_1(A_LineNumber, A_ThisFunc, Left, Right, i, Arr.Has(i) ? GetValue() : '', 'Return nothing - this shouldnt happen. i: ' i)
        }
        _GetNearest_L2R() {
            loop IndexEnd - i + 1 {
                if Arr.Has(i) {
...
Debug_Lines_1(Ln, Fn, Left, Right, i, Gv, Action) {
    global qf_debug
    if qf_debug.lines is Array
        qf_debug.lines.push(Format('{:4}: ', LN) Fn '; ' Action '; Left: ' Left '; Right: ' Right '; i: ' i '; GetValue(): ' Gv)
}
Debug_Lines_2(Ln, Fn, Msg) {
    global qf_debug
    if qf_debug.lines is Array
        qf_debug.lines.push(Format('{:4}: ', LN) Fn '; ' Msg)
}

if A_LineFile == A_ScriptFullPath {
    qf_debug_test := true
if IsSet(qf_debug_test) {
    Content := FileRead(A_ScriptFullPath)
    f := FileOpen(SubStr(A_ScriptFullPath, 1, -4) '_backup.ahk', 'w')
    f.Write(Content)
    f.Close()
    f := FileOpen(A_ScriptFullPath, 'w')
f.Write(RegExReplace(Content, 'mi)^( +)([^;].*?debug.*)', '$1; $2'))
    f.Close()
} else {
    Content := FileRead(A_ScriptFullPath)
    f := FileOpen(SubStr(A_ScriptFullPath, 1, -4) '_backup.ahk', 'w')
